...
from postflop import PostflopHistory
from preflop import PreflopHistory
#copy library may not be used

def getAction(strategy):
    return np.random.choice(list(strategy.keys()), p=list(strategy.values()))

class AIPlayer(PlayerType):
...
#What is observed_env???
# What happened to current_bet

class CFRAIPlayer(AIPlayer):
    def __init__(self, balance) -> None:
        super().__init__(balance)

        self.preflop_infosets = joblib.load("../python_skeleton/preflop_infoSets_batch_19.joblib")
        self.postflop_infosets = joblib.load("../python_skeleton/postflop_infoSets_batch_19.joblib")
    ...
    def get_action(
        ...
        if len(community_cards) == 0:
            ...
            infoset_key = "".join(PreflopHistory(abstracted_history).get_infoSet_key())
            ...
            infoset_key = PostflopHistory(abstracted_history).get_infoSet_key_online()
